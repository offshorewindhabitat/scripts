...
tif_to_cogs <- function(in_tif, redo=F, upload_to_gcs=F){
  ...
  if (all(fs::file_exists(c(rel_tif, prs_tif)) & !redo)){
    message("  SKIPPING; both rel_tif & prs_tif exist")
    return(TRUE)
  }
...
d_cl <- d_lyrs |> 
  ...
  partition(cl) |> 
  mutate(
    do_cog = map_lgl(path_tif, tif_to_cogs)) |> 
  collect()
```
...
# * write to duckdb table lyr_zone_stats ----
...
oh_block_stats <- function(tifs, stat="mean", csv, redo=F){
  # tifs    = d_lyrs %>% slice(1:5) %>% pull(path_tif)
  # stat    = "mean"
  # dir_csv = dir_zs
  # redo    = F
...
d_cl <- d_lyrs |> 
  ...
  mutate(
    mean_batch_csv = map_chr(
      ...
      \(x){
        glue::glue(
        "{dir_zs}/block_batch{paste(range(x$rowid), collapse='-')}_mean.csv")
        })) |> 
  partition(cl) |> 
  mutate(
    do_mean     = map2_lgl(
      data, mean_batch_csv, 
      \(d, csv) oh_block_stats(d$path_tif, stat="mean", csv=csv))) %>% 
  collect()
