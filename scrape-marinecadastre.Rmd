...
```{r scrape}
librarian::shelf(
  DT, dplyr, glue, purrr, readr, rvest, stringr, tibble)
  # tidyverse, webshot)

url_mc       <- "https://marinecadastre.gov/data/"
dir_pfx      <- "/Users/bbest/My Drive/projects/offhab_data"
dir_mc       <- glue("{dir_pfx}/marinecadastre.gov")
csv_mc       <- glue("{dir_mc}_datasets_{Sys.Date()}.csv")
csv_mc_paths <- glue("{dir_mc}_datasets_paths.csv")
csv_mc_gh    <- glue("{dir_mc}_datasets_marinecadastre.gov.csv")
...
read_csv(csv_mc) %>%
  select(-description,-downloads, -metadatas) %>%
  # select(-description) %>%
